export class Component extends HTMLElement{constructor(parent,x,y){super();x=x||0;y=y||0;this._enabled=true;this.attachShadow({mode:"open"});this.style.position="absolute";this.move(x,y);if(!parent){return}if(parent instanceof Panel){parent.addChild(this)}else{parent.appendChild(this)}}move(x,y){this.x=x;this.y=y}setSize(w,h){this.width=w;this.height=h}get x(){return this._x}set x(x){this._x=x;this.style.left=x+"px"}get y(){return this._y}set y(y){this._y=y;this.style.top=y+"px"}get width(){return this._width}set width(w){this._width=w;this.style.width=w+"px"}get height(){return this._height}set height(h){this._height=h;this.style.height=h+"px"}get enabled(){return this._enabled}set enabled(enabled){this._enabled=enabled}}customElements.define("minimal-component",Component);export class Button extends Component{constructor(parent,x,y,text,defaultHandler){super(parent,x,y);this._text=text;this._defaultHandler=defaultHandler;this.createChildren();this.createStyle();this.createListeners();this.setSize(100,20)}createChildren(){this.button=document.createElement("div");this.button.setAttribute("class","MinimalButton");this.button.tabIndex=0;this.label=document.createElement("div");this.label.textContent=this._text;this.label.setAttribute("class","MinimalButtonLabel");this.button.appendChild(this.label);this.shadowRoot.append(this.button)}createStyle(){const style=document.createElement("style");style.textContent=`
  ...
  `}export class TextArea extends Component{constructor(parent,x,y,text,defaultHandler){super(parent,x,y);this._text=text;this._defaultHandler=defaultHandler;this.createStyle();this.createChildren();this.createListeners();this.setSize(100,100)}createChildren(){this.textArea=document.createElement("textArea");this.textArea.setAttribute("class","MinimalTextArea");this.textArea.value=this._text;this.textArea=this.textArea;this.shadowRoot.append(this.textArea)}createStyle(){const style=document.createElement("style");style.textContent=`
    ...
    `;this.shadowRoot.append(style)}onMouseDown(event){this.offsetY=event.clientY-this.getBoundingClientRect().top-this.handle.offsetTop;if(this.offsetY<0||this.offsetY>this.handleSize){this.offsetY=this.handleSize/2;let y=event.clientY-this.getBoundingClientRect().top-this.handleSize/2;this.calculateValueFromPos(y)}document.addEventListener("mousemove",this.onMouseMove);document.addEventListener("mouseup",this.onMouseUp)}onMouseMove(event){let y=event.clientY-this.getBoundingClientRect().top-this.offsetY;this.calculateValueFromPos(y)}updateHandlePosition(){let percent=(this.value-this.min)/(this.max-this.min);percent=Math.max(0,percent);percent=Math.min(1,percent);this.handle.style.top=this.height-this.handleSize-percent*(this.height-this._handleSize)+"px"}updateLabelStyles(){if(this.label){this.label.style.left=(-this._labelWidth+this.width)/2+"px";this.label.style.top="-15px";this.label.style.width=this._labelWidth+"px"}if(this.valueLabel){this.valueLabel.style.width=this._labelWidth+"px";this.valueLabel.style.left=(-this._labelWidth+this.width)/2+"px";this.valueLabel.style.top=this.height+5+"px"}}updateSliderSize(w,h){this.setSize(h,w)}calculateValueFromPos(y){const percent=1-y/(this.height-this.handleSize);const value=this.min+(this.max-this.min)*percent;this.updateValue(value)}get handleSize(){return this._handleSize}set handleSize(handleSize){this._handleSize=handleSize;this.handle.style.height=handleSize+"px";this.updateHandlePosition()}}customElements.define("minimal-vslider",VSlider);